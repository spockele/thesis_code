...
# ----------------------------------------------------------------------------------------------------------------------
# Post-Processing of Spectrogram output of reception model
# ----------------------------------------------------------------------------------------------------------------------
#     spectrogram = pd.read_csv(os.path.join(proj.project_path, 'spectrograms', 'spectrogram_tst.csv'),
...
#
#     plt.figure(1)
#     ctr = plt.pcolor(spectrogram.columns, f, 20 * np.log10(np.abs(x_fft.T) / hf.p_ref), vmin=-60, vmax=60)
...
#     ctr = plt.pcolor(t_stft, f_stft, 20 * np.log10(np.abs(x_stft) / hf.p_ref), vmin=-60, vmax=60)
#     cbar = plt.colorbar(ctr)
#     plt.xlabel('t (s)')
#     plt.ylabel('f (Hz)')
#     cbar.set_label('PSL (dB / Hz)')
...
#     spio.wavfile.write('055ms_rec0.wav', int(f_s_desired), wav_dat)
